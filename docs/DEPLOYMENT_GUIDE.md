...
echo "deb [arch=amd64 signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/ubuntu $(ls_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
...
# Set up stable repository
echo "deb [arch=amd64 signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/debian $(ls_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
